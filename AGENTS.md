...
1. **Create the component** in `src/components/<Name>.tsx` — a standard React component.
2. **Export from barrel** — add the re-export in `src/components/index.ts` and add the name to `BUILTIN_COMPONENT_NAMES` in `src/components/builtin-registry.ts`.
